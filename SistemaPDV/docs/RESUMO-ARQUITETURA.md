...
| `Cliente` | Cadastro de clientes — sincronizado da API, inclui o "Consumidor Final" (id 1) pra vendas avulsas. Guarda o endereço completo e dois códigos de cidade: `CidadeId` (número INTERNO da API, ex: 1336) e `CodigoCidade` (o do IBGE, ex: 2507507 — o `c_cidade` do cadastro). |
...
| `CaixasApiService` | Consulta (`GET .../financeiro/caixa-funcoes`, uma página por chamada) dos caixas que a API tem, para a tela de Configurações: monta a URL com os filtros (`data_inicial`, `data_final`, `fechado=1`, `per_page`, `page`), resolve `operador_id` pelo cadastro local (`Funcionario.IdExterno`; sem cadastro vira "Operador N") e aceita a resposta em objeto ou embrulhada num array. Só leitura. |
| `CepService` | Consulta de CEP no ViaCEP (`https://viacep.com.br/ws/{cep}/json/`): devolve logradouro, bairro, cidade, UF e o código IBGE da cidade (o `c_cidade` da API). Só conveniência: falha (sem internet, CEP inexistente, resposta estranha) nunca lança e orienta a preencher à mão. O único dado que sai do aparelho é o CEP. |
...
| `ClienteFormPainel` (controle) | O modal "Cadastrar Cliente no Banco Local" de Cadastros (aberto por "Novo Cliente"): Nome e CPF/CNPJ obrigatórios; telefone, e-mail e o endereço completo opcionais. No bloco de endereço (`EnderecoFormViewModel`, o `NovoEndereco` do `CadastrosViewModel`) o botão Buscar consulta o CEP, preenche logradouro, bairro e cidade e guarda o código IBGE da cidade; o corpo rola em janelas baixas. Enter salva (no CEP, busca), Esc cancela; a tela de trás fica desabilitada. Só grava local — o envio é da fila. Usa os estilos globais `TextBox.campo` e `TextBlock.rotuloForm`. |
| `ProdutoFormPainel` (controle) + `ProdutoFormViewModel` | O modal "Cadastrar Produto no Banco Local" de Cadastros (aberto por "＋ Novo Produto"): nome, SKU/código, categoria (combo dos grupos sincronizados), preço de venda e de custo (opcional). Sem campo de estoque — a API não grava estoque no cadastro (#86, #94). O ViewModel é um filho do `CadastrosViewModel` (`FormProduto`); `ModalAberto` desabilita a tela de trás quando qualquer dos dois modais (cliente ou produto) está aberto. Só grava local — o envio é da fila. |
| `CaixasApiPainel` (controle) + `CaixasApiViewModel` | O modal "Caixas no SoftcomShop" (botão em Configurações): lista SÓ PARA LEITURA os caixas que a API tem, de qualquer PDV da empresa — filtro por data (dd/mm/aaaa), "somente fechados" e paginação de 15. Abre já consultando (sem filtro a API devolve os últimos 7 dias). Filho do `ConfiguracoesViewModel` (`Caixas`); precisa de rede e não grava nada local. |
...
| `PainelFilaOutbox` (controle) | O painel lateral da fila outbox (abre pela pílula "Sync: N pendentes" da barra do topo): o cartão de estado da fila (🟢 tudo enviado / 🟡 aguardando / 🔄 sincronizando, com barra de progresso e a etapa / 🔴 sem conexão — `EstadoDaFila`), as pendências por tipo em chips (`PendenciasPorTipo`), o botão "Disparar Sincronização Agora" (fica parado, escrito "Sincronizando…", enquanto um envio dura) e o log de atividade em blocos por ciclo (cabeçalho, itens com ícone por nível e resumo colorido). A tela de baixo fica desabilitada (e os atalhos dela dormem) enquanto ele está aberto; fecha com ✕, clique no fundo ou Esc. |
...
| `LogDeSincronizacao` / `EntradaDeLog` | As últimas 100 atividades da sincronização em segundo plano, na memória, escritas para o OPERADOR ler (conexão que caiu/voltou, "Pedido #1003 sincronizado com sucesso!", o que foi recusado e por quê). Cada ciclo que envia algo é um bloco: `Registrar(..., MarcaDeLog.InicioDeCiclo)` abre, `FimDeCiclo` fecha com o resumo ("Ciclo concluído em 1,2 s: 3 enviado(s), 1 com falha."); `AndamentoDoEnvio(Sincronizando, Etapa)` diz em tempo real qual etapa está saindo (o App leva ao `ShellViewModel.DefinirAndamento`). Quem sincroniza chama `Registrar` de qualquer thread; o App leva as linhas à thread de interface (`ShellViewModel.AdicionarAtividade`). Não é o log técnico: esse é o arquivo do `Registro` (com exceções); aqui nunca entra token, senha nem detalhe de exceção. |
