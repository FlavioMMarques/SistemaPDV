...
**Verificação contra a API real e contra dados de verdade (cópia do banco, só leitura na API):** (1) a migration `AddGrupos` foi aplicada numa cópia de um banco existente (201 produtos, 37 clientes, 14 funcionários, 5 vendas): todos os dados sobreviveram e não sobrou migration pendente; (2) o serviço REAL de grupos rodou contra a API: 1º ciclo trouxe 50 grupos, 2º ciclo "mudou 0" (o ciclo de 5 min não recarrega telas à toa), e **os 201 produtos disponíveis ganharam categoria** (25 categorias distintas). Nada foi escrito na API nem no banco do usuário.

**Revisão de código de Cadastros/Grupos (skill `code-review-and-quality`).** (1) **Achado corrigido — apagar por um soluço da API:** a sincronização por substituição trata "a API sumiu com o grupo" como "apague". Uma resposta 200 com lista VAZIA (permissão, filtro, servidor reiniciando) de uma empresa que já tinha grupos apagaria todas as categorias até o próximo ciclo (5 min), e os cards do PDV voltariam à unidade. Agora resposta vazia com grupos já conhecidos é tratada como suspeita: mantém o que existe e tenta de novo no próximo ciclo (uma empresa com produtos não fica sem nenhum grupo); teste dedicado. Os cartões seguem com a regra antiga (lista pequena e de outra natureza — não mexi sem pedir). (2) **Conferido no renderizador clicando de verdade:** os três botões de aba passam o enum como parâmetro do comando, a aba atual e a classe "ativa" trocam certo, e "Cadastrar Cliente" leva à aba de clientes com o formulário aberto (nos testes de ViewModel os comandos eram chamados direto, sem a ligação do XAML). (3) **Processo:** ao corrigir um aviso de compilação editei a linha errada duas vezes e apaguei uma asserção; a cadeia `build | grep && commit` não parou porque o `grep` escondeu o código de saída do build, e dois commits saíram quebrados. Os commits ainda eram locais e foram juntados num só; daqui em diante o commit só sai depois de conferir o **código de saída** do build (não a saída filtrada) e de rodar a suíte. (4) **FYI, sem mudança:** (a) se os grupos chegam pela 1ª vez com a tela de venda já aberta, os cards mostram a unidade até o operador sair e voltar (a tela de venda não recarrega o catálogo no meio de um carrinho, de propósito); (b) uma falha ao baixar os grupos deixa a pílula "Online (com falhas)" como os cartões; (c) a ordenação por nome no SQLite não considera acento (mesmo comportamento das outras listas). **Sem achados** em segurança (o resumo do operador não tem CPF nem chave; teste) nem em desempenho (1 requisição de grupos por ciclo; 4 consultas por recarga da tela).

## 81. Produto sem preço (R$ 0,00): a regra mora em UM lugar e o serviço ainda a repete por segurança

**Onde:** `PdvViewModel.AdicionarItem`, `Views/Controls/PrecoProdutoPainel`, `VendaService`, `ItemCarrinho`, `Produto.SemPreco` (tarefa anotada em 2026-09-21)

- **Decisões do usuário:** o preço tem de ser **maior que zero** (nada de item de graça por engano — "brinde" ficou de fora de propósito) e **qualquer operador** pode informá-lo (a chave de supervisor está desligada; se ela voltar, este é um ponto a rever). O preço digitado vale **só para aquele item do cupom**: o cadastro não muda, e é o preço que a venda grava em `ItemVenda.PrecoUnitario` — que já é o `preco` enviado à API (teste confere a venda gravada; o corpo enviado sai desse mesmo campo, ver #78).
- **Um ponto de entrada.** Todos os caminhos de lançamento (clique no card, Enter da busca, leitor de código de barras) passam por `AdicionarItem`; a checagem `PrecoVenda <= 0` mora ali e abre o painel em vez de lançar. Testes cobrem cada caminho. Como o item **não entra** até o preço vir, o aviso "adicionado ao cupom" também só sai depois.
- **O painel é um modal como o de pagamento:** a tela de trás fica desabilitada (`ModalAberto` = pagamento OU preço), o foco vai para o campo de preço, **Enter lança e Esc cancela** (o Esc só descarta o lançamento — o cupom que já existia fica; verificado), e **F10 não paga por baixo** do painel (F10 só age sem o painel de preço). Novo (F2) descarta painel e cupom. Ao fechar, o foco volta à busca (o leitor de código de barras segue digitando). Verificado no renderizador com teclas de verdade: foco no campo → "0" + Enter recusa com o motivo e continua aberto → "12,50" + Enter lança e devolve o foco à busca.
- **Validação do valor:** só maior que zero; a leitura é a mesma do dinheiro (vírgula ou ponto; mais de 2 casas e "1.234" são recusados por ambíguos — dinheiro errado em silêncio é pior que pedir de novo); teto de sanidade de R$ 1.000.000 (acima disso é erro de digitação; o teto em si passa). Não há como saber que "1250" era "12,50": o total do cupom fica à vista para o operador conferir.
- **Transparência:** o cupom marca o item como "preço informado" (para ninguém achar que é preço de tabela) e o card do produto mostra "Informar preço" no lugar de um "R$ 0,00" que pareceria defeito.
- **Rede de segurança no domínio:** `VendaService.RegistrarVendaLocalAsync` passou a recusar (exceção) qualquer item com preço ≤ 0. A tela já impede, mas a venda gravada é o dado que vai ao fiscal/API: a regra não pode depender de todo caminho de código lembrar dela. Nenhum teste existente dependia de item de preço zero.
- **Pegadinha do renderizador (de novo):** um painel que já nasce visível não dispara o "ficou visível" que leva o foco para o campo; por isso o teste de teclado abre o painel DEPOIS de mostrar a janela, como no app.
