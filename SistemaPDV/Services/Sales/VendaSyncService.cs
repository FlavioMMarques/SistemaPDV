...
public class VendaSyncService
{
    ...
    public async Task<ResultadoSincronizacaoRecurso> SincronizarVendaAsync(Guid vendaId, string accessToken, CancellationToken ct = default)
    {
        ...
        // A requisição (URL + corpo) é montada por MontadorDeRequisicaoDeVenda — o mesmo código que o modal "Detalhes do
        // pedido" usa para mostrar o que é enviado. Faltando uma dependência (funcionário, empresa, cliente, produto, forma
        // de pagamento) a venda ESPERA: continua PendenteSync, sem contar tentativa, com o motivo gravado.
        var montagem = await MontadorDeRequisicaoDeVenda.MontarAsync(context, venda, ct);
        if (montagem.Requisicao is not { } requisicao)
            return await AguardarAsync(context, venda, montagem.Espera!, ct);

        var resultado = await apiClient.EnviarAsync(HttpMethod.Post, requisicao.Url, requisicao.Corpo, accessToken, ct);
