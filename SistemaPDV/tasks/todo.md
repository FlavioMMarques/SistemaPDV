...
7. **Fechamento de caixa com vendas ainda pendentes (decidir).** O outbox envia abrir -> vendas -> fechar, mas o fechamento vai mesmo que alguma venda do caixa tenha FALHADO ou esteja em espera crescente; a API fecharia o caixa sem essas vendas. Opções: (a) o fechamento espera enquanto houver venda do caixa pendente/em espera (com motivo visível), o que pode travar o fechamento se uma venda tiver erro permanente — a saída seria "Reenviar falhas" ou cancelar a venda; (b) manter como está e avisar na tela de fechar caixa quando há vendas não sincronizadas; (c) bloquear o botão "Fechar caixa" localmente enquanto houver vendas com falha. Não mexi: é regra de negócio (o que a API faz com vendas que chegam DEPOIS do fechamento também não foi testado).
8. **Fechar caixa sem apuração de bandeiras de cartão** (`digitacao_bandeiras` vai vazia) e o 1º envio real do fechamento já foi aceito pela API (caixa 28 com `data_fechamento`), mas o POST de fechar ainda pode pedir campos que só aparecem em outro cenário (ex: apuração de cartão).
