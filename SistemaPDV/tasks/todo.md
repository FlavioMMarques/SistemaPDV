...
1. ~~Login bcrypt sem limite de tentativas~~ — **feito (2026-09-20), ver APRENDIZADOS #70.** Texto original: Confere a chave contra cada funcionário ativo com hash (custo 10 ≈ dezenas de ms cada; ~1 s no pior caso com 14 operadores; roda fora da thread de UI). Não há bloqueio/atraso após erros repetidos — o bcrypt lento já dificulta força bruta, mas um PDV exposto poderia ganhar um atraso progressivo. Se o nº de operadores crescer (dezenas), medir o tempo de login.
2. ~~`NumeroPedido` = MAX + 1~~ — **feito (2026-09-20): colisão tenta o próximo número (até 5x), ver APRENDIZADOS #70.** Texto original: Seguro hoje (o botão Finalizar não deixa duas vendas simultâneas no mesmo PDV; o índice único impediria duplicar) — só colide com **dois processos no mesmo banco**. Se colidir, a `DbUpdateException` derruba o registro da venda; considerar 1 nova tentativa. (A unicidade por empresa foi resolvida com o código do PDV — item 10.)
3. ~~`CatalogSyncService` com ~600 linhas~~ — **decidido pelo usuário (2026-09-20): deixar como está.** A classe segue com 602 linhas (os cartões ficaram num arquivo `partial` à parte) e a divisão só volta a valer se ela crescer perto do limite (~1000). Texto original: **`CatalogSyncService` com ~600 linhas** e duas responsabilidades (catálogo + envio de cliente novo). Longe do limite (~1000), mas é o candidato a dividir (ex: `ClienteSyncService`) se crescer.
...
8. ~~Fechar caixa sem apuração de bandeiras de cartão~~ — **feito (2026-09-20, Fase 6)**: adiado de manhã e reativado pelo usuário quando a rota de cartões foi achada. Texto original: (`digitacao_bandeiras` vai vazia) e o 1º envio real do fechamento já foi aceito pela API (caixa 28 com `data_fechamento`), mas o POST de fechar ainda pode pedir campos que só aparecem em outro cenário (ex: apuração de cartão).
...
## Fase 6 — Cartões e bandeiras (2026-09-20, pedido do usuário)

**Objetivo:** "sincronizar os cartões; ao colocar a forma de pagamento cartão, habilitar a escolha da bandeira; e no fechamento informar os valores por bandeira". A rota `GET softauth/api/financeiros/cartoes/page/N` (SEM `v2`, envelope próprio, tudo como texto) foi achada com a dica de olhar o caminho do token. Ver APRENDIZADOS #71.

**Decisões (do usuário: "você que decide"):** bandeira guardada como TEXTO no pagamento da venda (sem FK: a venda é histórica); a lista de bandeiras = nomes distintos dos cartões sincronizados; escolha obrigatória só quando a forma é cartão **e** há bandeiras sincronizadas (sem cartões não trava a venda); o fechamento mostra uma linha por bandeira que aparece nas vendas do caixa, pré-preenchida com o esperado; cartões sincronizados por SUBSTITUIÇÃO (só se a busca inteira der certo).

- [x] **Task 60 — Dados:** `Cartao` (sem CNPJ), `PagamentoVenda.Bandeira`, migration `AddCartoesEBandeiraNoPagamento`.
- [x] **Task 61 — Sincronização:** `SoftcomApiClient.BuscarPaginasAsync` (paginação por número, sem seguir URL do servidor, trata "Invalid pagination interval" na 1ª página como vazio), `CatalogSyncService.SincronizarCartoesAsync` (partial), entra no ciclo/status/`TudoComSucesso`. 15 testes.
- [x] **Task 62 — Venda:** `CatalogoLocalService.ListarBandeirasAsync`, `VendaService` com bandeira por pagamento (overload; a chamada antiga segue valendo), `PdvViewModel`/`PdvView` (ComboBox só com bandeiras sincronizadas; obrigatória para cartão), `PagamentoAlocado.Descricao`. 14 testes.
...
- [x] **Task 72 — Painel lateral da Fila Outbox**: pílula "Sync" vira botão que abre o painel (pendências, Disparar Sincronização Agora, log de atividade em linguagem de operador: conexão, pedidos enviados/recusados com motivo, caixa, catálogo). Log em memória (100 linhas), sem token nem detalhe técnico; Esc fecha sem cancelar a venda por baixo. Conferido por renderização; 765 testes verdes.
- [x] **Task 73 — Cadastros**: 3 abas (Produtos com categoria, Clientes com ID/telefone/cidade, Operadores de Caixa), contagens totais, Novo Cliente (formulário recolhido) e Novo Produto desabilitado. Grupos/categorias sincronizados (`GET softauth/api/v2/produtos/grupos`, rota conferida na API real só com leitura) + migration AddGrupos; card do PDV mostra a categoria. Conferido por renderização; 799 testes verdes. **Fase 8 completa.**
