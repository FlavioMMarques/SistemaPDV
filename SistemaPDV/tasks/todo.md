...
- [ ] **Confirmar com a API real (só o usuário/autorização):** (a) o fechamento com `digitacao_bandeiras: [{bandeira:"MASTERCARD", valor:…}]` é aceito — o formato (nome, não id) veio do Swagger e nunca foi testado; (b) a venda não envia a bandeira à API (o contrato do POST de venda para isso é desconhecido) — só o fechamento a usa.
